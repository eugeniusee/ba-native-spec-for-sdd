...
4. **Pre-flight** — the six CC-H assertions restricted to `deps(F)`. This is the
   hard guarantee, run fresh every time; the ledger head in
   ...
   CC-H-05) to the `ba-gate` agent with `assertions-h.md`, both against the
   snapshot workspace.
...
3. **The gate stops.** Nothing past this line is the gate's act. The adapter
   (`/ba-handoff`) owns the plumbing and the hash guard; the operator owns the
   pipeline from `/speckit-plan`; the BA re-enters at Band-3 verification.
