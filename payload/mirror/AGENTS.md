...
A spec that reached `/speckit-plan` was certified by the gate. **Do not edit a
certified spec to make implementation easier** — spec errors are fixed in the
spec and re-run downstream, never hand-patched in code.
<!-- ba-native-spec:end -->
