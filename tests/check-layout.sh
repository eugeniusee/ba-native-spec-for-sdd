...
# 39 /ba-* skills registered, manifest vector correct.
#
...
# ─────────────────────────────────────────────── the 39 /ba-* skills, by name ──
printf '\n\033[1m▸ /ba-* skill registry — 39 expected (build plan §2.9)\033[0m\n'
...
if [ "$expected_skills" -ne 39 ]; then
  bad "layout.expected registers $expected_skills /ba-* skills, not 39 — the manifest itself is wrong"
else
  pass=$((pass+1))
  note "layout.expected registers all 39 by name (19 workflow + 20 technique)"
fi

if [ "$found_skills" -eq 39 ]; then
  ok "39 of 39 /ba-* skills installed"
elif [ -n "$SESSION" ]; then
  printf '  \033[33m○\033[0m %s of 39 /ba-* skills installed — %s pending, itemized below\n' \
    "$found_skills" "$((39 - found_skills))"
else
  bad "$found_skills of 39 /ba-* skills installed — the full tree is the Phase-2 bar"
fi
