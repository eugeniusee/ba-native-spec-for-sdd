...
# 36 /ba-* skills registered, manifest vector correct.
#
...
# ─────────────────────────────────────────────── the 36 /ba-* skills, by name ──
printf '\n\033[1m▸ /ba-* skill registry — 36 expected (build plan §2.9)\033[0m\n'
...
if [ "$expected_skills" -ne 36 ]; then
  bad "layout.expected registers $expected_skills /ba-* skills, not 36 — the manifest itself is wrong"
else
  pass=$((pass+1))
  note "layout.expected registers all 36 by name (16 workflow + 20 technique)"
fi

if [ "$found_skills" -eq 36 ]; then
  ok "36 of 36 /ba-* skills installed"
elif [ -n "$SESSION" ]; then
  printf '  \033[33m○\033[0m %s of 36 /ba-* skills installed — %s pending, itemized below\n' \
    "$found_skills" "$((36 - found_skills))"
else
  bad "$found_skills of 36 /ba-* skills installed — the full tree is the Phase-2 bar"
fi
