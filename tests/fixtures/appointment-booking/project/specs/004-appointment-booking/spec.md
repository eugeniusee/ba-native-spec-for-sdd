...
Scenario: Two Clients race for the last Slot
  Given Specialist "Dr. Ivanova" has one published Slot at 2026-07-20 10:00
  And Client A and Client B both have that Slot open in the booking surface
  ...
  And Client B sees Dr. Ivanova's published Availability without the 10:00 Slot
```
...
FR-007 (US1) — WHEN an Appointment is created, THE SYSTEM SHALL notify the
Specialist of the new Appointment.
...
NFR-003 (performance) — Notification of a confirmed Appointment reaches the
Specialist within 60 seconds of the confirmation, under normal load.
