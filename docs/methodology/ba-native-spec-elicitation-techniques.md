...
### BA-Native Spec · elicitation engine · v0.11 — principle 4 carries one test: necessity seats, a request is a flag (4 Sep 2026)
**v0.11 change record:** one sentence, one principle, ruled 4 Sep 2026 (**D15**, §17; origin: **EC-24** — the field feedback of the AI-SDD team sync of early September 2026, triaged at `docs/field-notes/2026-09-04-first-phase-composed-wide.md`: the Nutrivity run's first phase came out wider than the BA's cut with every row legitimate, because principle 4's second test — *hard-requested* — seated any epic a `[stated]` citation covered, and a client brief hard-requests in breadth; the allocation-grain ruling is orchestrator v0.45's — D-O104–D-O106, §48 — and the consumer catalogue-b6 v0.9's — D-B6-18–D-B6-20): **the principle carries one test** — what enters the first phase, an essential-scope set, or a story set passes the **necessity test**, the stated business goal cannot be reached without it; a hard request the goal does not need is **recorded ground and a named candidate, never a seat** — a cited row or a deferred line with the request on the record, named by the composing run so the BA may direct it in; uniform across the three grains on purpose, a lean phase composed of fat epics being no lean phase. D13's controlling-statement sentence stands with one word moved. No template, guard or skill brief changes by this edition; the Lane A pass compiles the sentence where the principle is cited.

**v0.10 change record:** one decision, one check, ruled 25 Aug 2026 (**D14**, §16; **R7** of the EC-21 package — the field design-defect report of 25 Aug 2026, *the gate does not distinguish evidence from decision*, and its verified triage `docs/field-notes/2026-08-22-cc-fl-04-coverage-inversion.md`: a Tier-2 run in assumption posture minted an `[ASSUMED: …]` tag in 45 error-table cells across 25 draft specs, a form this engine never defined, and nothing between the writer and the gate caught it — the gate's own stub test then read the mint as content): **the shape pre-flight gains a fifth check** (§5.3 step 4) — every bracketed token of marker shape in the draft is the spec's one marker, `[NEEDS CLARIFICATION: …]` (standard rule 7), and a mint **stops the write** with found vs expected, at authoring, where it is one edit — rather than at the gate, where it is a non-waivable CC-G-02 line (contract v0.4). The check is on the standard's own law exactly as the four before it, and it asserts nothing the gate does not already assert: **tolerance is for specs the framework did not write, discipline is for the ones it did** (D11's own boundary, applied). §7.2's build requirement names the fifth check. §5.3's cite-or-mark and the confidence rule stand as written — the check enforces their form, it does not restate them; D11's *four checks* is amended to five here, on the record. **No guard (§6), no question-legality rule (D6), no cap (D7), no depth calibration (§3.3), no signal class, no brief-template section, no assertion, no new BA step and no new artifact.** Companions: writing standard v0.6 (rule 7 — the grammar this check reads) · completeness contract v0.4 (CC-G-02 — the line it pre-empts) · gate v0.12 (§5.2).

**v0.9 change record:** one companion mirror, ruled 21 Aug 2026 (**R2** of the EC-18 package; origin: the field defect report of 20 Aug 2026, **Part B item B6**; the law moved at **writing standard v0.5 §3**, which is now the role rule's one home): §5.2's context-stack row 3 and §5.3 step 1 stop stating *"roles from `roles-permissions.md` only"* as a rule of their own and **cite §3's two branches** — the verbatim rule where the file exists, the canvas-actor-plus-marker rule where the standing profile leaves its producing technique out of profile. The stack **order, membership and precedence are untouched**, and the governance-wins precedence clause stands verbatim: a brief still cannot redefine a role. **No question rule, no guard, no principle, no output contract and no template moved** — D6, D7, the two guards and principle 4 each stand as v0.8 left them; the §7.2 build brief keeps its requirements and gains the same citation.
**v0.8 change record:** one sentence, one principle, ruled 18 Aug 2026 (**D13**, §15; origin: orchestrator v0.27 §30 — EC-10, the scope-decision harvest D-O65 and the precedence principle D-O66; the two legitimacy tests live here and catalogue-b6 cites them without re-legislating, so the rule for *reading* them lands here, once): **principle 4 gains its controlling-statement sentence** — the two tests read a **negotiated engagement-scope decision** (orchestrator §8.1, D-O65–D-O66) as the **controlling client statement** where it conflicts with an earlier request in the material on hand; **composition-bounding only** — discovery is untouched, and a trimmed capability remains a cited row. Additive at the principle's own line; no renumbering, no disturbed citation, no section, guard, template row or skill brief moved. Companions: orchestrator v0.27 (D-O65–D-O67) · catalogue-b6 v0.5 (D-B6-12–D-B6-13 — the reading rule that cites this sentence) · catalogue-b1 v0.6 (D-B1-7).
**v0.7 change record:** one decision, one section shape, ruled 17 Aug 2026 (**D12**, §14; origin: the shape-guard set of the same day — orchestrator v0.23's D-O58 pins a dashboard reader that counts brief §6 rows off-shape and names them, and the grammar that reader matches was carried by the skeleton and the worked example without ever being **stated as law**): **brief §6's Open-Question IDs are `OQ-<n>`** — OQ-1, OQ-2, … **numbered per brief**, the number being that brief's own sequence, restarting in every brief and **never globally unique** · **epic context rides beside the ID in a render, never inside it** — the epic is the brief's own and every reader already carries it, so an epic-qualified ID would encode twice what is known once. Stated **once**, in §4's brief skeleton where the section's shape is pinned; the payload template and the Tier-1 skill carry the same shape and cite it, and the worked example was already conformant (`OQ-1` · `OQ-2`) — **no example was rewritten to fit the rule**. **No reader change and no new reader:** D-O58's line-4 application reads this grammar and is legislated in the orchestrator, not here. **No guard (§6), no question-legality rule (D6), no cap (D7), no depth calibration (§3.3), no status vocabulary (D4), no signal class, no other brief section, no assertion and no new BA step:** CC-XA-06 reads the Open rows exactly as it did. Companion: orchestrator v0.23 (**D-O56–D-O59**, §27) — the shape-guard set this grammar serves.

**v0.6 change record:** one decision ruled 14 Aug 2026 (**D11**, §13; origin: a live Presale field report, 14 Aug 2026 — six drafted specs read by the shared parser as carrying nothing, because their `##` headings carried the §2 ordinals and their story IDs were bolded; the Tier-2 skill had instructed the author in prose to use *the standard's exact headings and exact order* and had never handed over the template file the checkers parse): **the skeleton is emitted from the template *file*** (§5.3 step 2) — the drafting act copies `.specify/templates/spec-template.md` and fills it in place, because a restatement of a skeleton is a description and descriptions drift · **the shape pre-flight** (new §5.3 step 4) — four checks on the standard's own law, run before the draft is written out, failing **loudly at authoring time** with the reader's found-vs-expected grammar. §7.2 gains both as build requirements. **No guard, no question-legality rule (D6), no cap (D7), no depth calibration (§3.3), no signal class, no brief-template section, no assertion and no new BA step:** the drafting act's *inputs* and *outputs* are unchanged — what changes is that the writer now works from the same bytes the reader does. Companions: writing standard v0.4 (§2's ordinal statement and reader-tolerance record; golden rule 4's table boundary) · orchestrator v0.18 (§10.4's unreadable spec — D-O50).
...
4. **Compose lean.** Generation acts compose **the minimal scope that achieves the stated business goal** — depth along the core journey, never breadth of coverage. Discovery stays coverage-complete; composition stays lean — what enters the first phase (`MVP`), an essential-scope set, or a story set passes the **necessity test**: the stated business goal cannot be reached without it (goal-blocking). A hard request the goal does not need is **recorded ground and a named candidate, never a seat** — it stays a cited row or a deferred line with its request on the record, and the composing run names it so the BA may direct it in (D15, §17; orchestrator D-O104 at allocation grain — the former second test, *hard-requested*, seated on paper nothing it should not have and in the field most of a client brief). **Recorded breadth is welcome; composed breadth is debt.** The pair is the whole of the principle: it is what reconciles a coverage-complete estate with a lean composition, and neither half stands without the other. The test reads a **negotiated engagement-scope decision** (orchestrator §8.1, D-O65–D-O66) as the **controlling client statement** where it conflicts with an earlier request in the material on hand — composition-bounding only; discovery is untouched, and a trimmed capability remains a cited row (D13, §15). Consumers cite this line; none restates it.
...
<ID grammar (D12, locked): OQ-<n> — OQ-1, OQ-2, … numbered per brief. The
number is this brief's own sequence: it restarts in every brief and is never
globally unique. Epic context rides BESIDE the ID in a render, never inside
it — the epic is the brief's own, and every reader already carries it, so an
epic-qualified ID would encode twice what is known once.
...
| 3 | `roles-permissions.md` | Story actors, per **standard §3's two branches** — verbatim from this file where it exists; from the canvas Core Functions actors, marked at first use, where the standing profile leaves T-12 out of profile (orchestrator §6.5) and the file does not. CC-US-02 is unlifted either way. Authorization tuples visible before drafting (CC-XA-01 awareness) |
...
1. **Stories first.** From the brief's Essential Scope lines covered by *this* feature (per §8 slicing), the **story-drafting skill** (built from scratch to the brief in §7.2) drafts user stories with acceptance — roles per **standard §3's two branches** (verbatim from `roles-permissions.md` where it exists; from the canvas Core Functions actors and marked at first use where the profile leaves its producing technique out of profile — cited, never restated), P1–P3 with ≥ 1 P1, stable US-IDs, tiered acceptance discipline. **Composed against the brief and nothing beyond it (principle 4):** the set covers the essential-scope lines this feature carries and stops there; an adjacent capability discovered while drafting routes to the brief's **Deferred** section (template §4, brief §3 — its existing home), never into a story. Under the Presale **assumption posture** — this section's draft-and-mark discipline run without client access (orchestrator §6.5) — the same clause is the anti-"end-to-end completion" guard: assumptions fill unknowns *inside* the essential scope; they never widen it.
2. **Skeleton around them.** The full standard-§2 skeleton is drafted: EARS FRs linked to stories, flows with error paths, feature NFRs per category, business rules, data tables, integration touchpoints (seeded from brief §4), Out of Scope (seeded from brief §3 + sibling slices), References. **The skeleton is emitted from the template *file*, never from a restatement of it.** The drafting act starts by copying `.specify/templates/spec-template.md` and filling it in place. A prose instruction to reproduce "the standard's exact headings and exact order" is not the skeleton — it is a *description* of the skeleton, and a description drifts: the writer works from the sentence and the reader works from the bytes, and nothing holds the two together. The file is the one artifact both sides share, so it is the one that is copied. Where the template and any restatement disagree, the template governs; a template the standard has outgrown is fixed at the template, not worked around in the draft.
...
4. **Shape pre-flight — before the draft is written out.** The draft is checked against the shapes the framework's readers actually parse, and a miss **stops the write loudly** rather than producing a file that reads as empty. Five checks, each on the standard's own law: every `##` heading is one of the ten, **unnumbered and in order** (§2 — the ordinals are the skeleton list's numbering, never typed into the document) · every story line starts with a bare `US<N> (P<1|2|3>) — `, **no `**` around the ID** (§3) · every functional requirement is a **line**, `FR-0NN (US<n>) — <EARS text>`, **never a table row** (§4, golden rule 4) · every BR and NFR carries its bare ID at the line start (§7, §8) · **every bracketed token of marker shape** — an upper-case label, with or without a colon — **is the spec's one marker, `[NEEDS CLARIFICATION: …]`** (rule 7; the canvas's `[CONFLICT: …]` is not a spec form): `[ASSUMED: …]`, `[TBD]` and any other mint stop the write here, one edit at authoring, instead of standing at the gate as a non-waivable CC-G-02 line (v0.10, D14). The pre-flight names what it found and what it expected, in the reader's own found-vs-expected grammar, and fails at **authoring** time — the one moment the author is present, holds the intent, and can fix the shape for free. **Why it exists:** the writer and the reader were coupled by description only, and six field specs drifted with nothing catching it. The reader's tolerances (standard §2) are a courtesy extended to specs already written; they are never a reason to relax this check, which is the discipline that keeps them unnecessary.
...
|---|---|
| Story drafting mechanics: strict *As a [role], I want [goal], so that [benefit]* with single-intent goals and non-restated benefits · role discipline grounded in project context, no invented roles · AC as testable one-condition bullets in system perspective, specific field names, no "etc.", 3–6 per story · sizing judgment: combine-when-logical, split-by-actor-step, no story too small/too large (> 8 AC ⇒ split) · per-epic-type story checklists (auth, CRUD, real-time, reporting, notifications) as recall aids | **Roles per standard §3's two branches** — verbatim from `roles-permissions.md` where it exists, from the canvas Core Functions actors and marked at first use where the standing profile leaves its producing technique out of profile (cited, never restated); the reference's soft "not 'user' if a more precise role is available" hardens to the standard's ban under both branches ("as a user" fails CC-US-02) · **P1–P3 priorities with ≥ 1 P1** (CC-US-03) — the reference has no priority model · **stable US-IDs, never reused** (CC-US-04) · **tiered acceptance** — the reference is checklist-only; the Gherkin tier with concrete data and the anti-re-narration rule (CC-AC-04) come from the standard §5 · **one capability per story** enforced strictly (CC-US-05) — "combine when logical" must never chain behaviors · **no ungrounded invention**: the reference's "(optional) enhancement" behavior is excluded — a bullet or story the brief doesn't ground is scope drift and a CC-XA-06 violation; a genuinely good idea becomes a routed finding or an Open Question, never silent scope · **output shape**: the spec.md §2 *User Stories* section — no standalone tables, no spreadsheet-oriented AC formatting · **glossary-canonical language + cite-or-mark** throughout |

**Two build requirements the field added (14 Aug 2026, D11):** the skill **hands over the template file** — it copies `.specify/templates/spec-template.md` and fills it in place, and never restates the skeleton in its own prose (§5.3 step 2); and it **runs the §5.3 step-4 shape pre-flight** before writing the draft out, failing loudly on a heading, story, FR, BR or NFR whose shape the framework's readers do not parse — and, since v0.10 (D14), on a bracketed token of marker shape that is not the spec's one marker. Both close the same gap: the writer and the reader were coupled by *description* — the skill instructed the author in prose to use "the standard's exact headings and exact order" and never handed over the file the checkers actually parse — and six live specs drifted with nothing catching it.
...
## 13. Review record (v0.5 → v0.6)

One decision ruled by the BA Lead, 14 August 2026 — **the writer/reader coupling**, applied as stated. Origin: a live Presale field report, 14 August 2026. Six drafted specs carrying stories, EARS requirements, NFRs, flows and out-of-scope sections were read as carrying nothing. The authoring habits behind it — the skeleton list's ordinals typed into the `##` headings, `**` around the story IDs — are exactly what a writer produces when handed a *description* of a skeleton instead of the skeleton: the ordinals are visible in the description, and nothing in the loop ever compared the result against the bytes a checker reads.

**What did not move.** No guard, no question-legality rule (D6), no cap (D7), no depth-calibration text (§3.3), no signal class, no brief-template section, no assertion, no new BA step and no new artifact. The Tier-2 output contract (§5.1) is unchanged — same spec, same sections, same cite-or-mark discipline. The pre-flight adds no question to the GQ packet and no stop to the BA's path: it is a check on the framework's own output, and its failure is the framework's to fix before the write, not the BA's to answer.

| # | Decision | Ruling (adopted as stated) | Where |
|---|---|---|---|
| **D11** | The writer and the reader, coupled by the file instead of by description | **Two clauses, one gap.** (1) **Emit from the template file** — the drafting act copies `.specify/templates/spec-template.md` and fills it in place; the skeleton is never restated in skill prose. Where template and restatement disagree the **template governs**, and a template the standard has outgrown is fixed at the template rather than worked around in the draft. (2) **The shape pre-flight** — before the draft is written out, four checks on the standard's own law: the ten `##` headings **unnumbered and in order** (§2) · bare `US<N> (P<1\|2\|3>) — ` story starts, no `**` around the ID (§3) · every FR a **line**, never a table row (§4, golden rule 4) · bare BR/NFR IDs at line start (§7, §8). It reports **found vs expected** and fails at **authoring** time — the one moment the author is present and the fix is free. **The reader's tolerances are never a reason to relax it:** standard §2's normalisations are a courtesy to specs already written, and this check is the discipline that keeps them unnecessary | §5.3 steps 2 and 4 · §7.2 |

**Conflict scan against standard v0.4 · contract v0.2 · gate v0.6 · orchestrator v0.18 · catalogues b1 v0.5, b2–b5 v0.2, b6 v0.4:** none found. The pre-flight asserts nothing the standard does not already fix, and adds no CC-assertion — it is an authoring-side check, and the gate's verdicts are untouched. It is deliberately **stricter than the reader**: a numbered heading passes sk_structure's tolerance and still fails the pre-flight, which is the intended asymmetry — tolerance is for specs the framework did not write, discipline is for the ones it did. **No erratum issued.**

## 14. Review record (v0.6 → v0.7)

One decision ruled by the BA Lead, 17 August 2026 — **brief §6's ID grammar**, applied as stated. Origin: the shape-guard set of the same day. The orchestrator's **D-O58** makes a dashboard reader count brief §6 rows **off-shape** when their first cell is not an `OQ-<n>` ID, and name the offender. That reader needs a law to match against — and the grammar it matches had never been one. It was carried by the skeleton's column header, by the worked example's `OQ-1`/`OQ-2`, and by the payload template, in three places and as law in none. **The defect class is the shape-guard set's own, one layer down: a grammar every reader depends on and no document states.**

**What did not move.** The **status vocabulary (D4)** is untouched — `Open` · `Answered — <date> → <destination>` · `Overtaken — <reason>`. **CC-XA-06** reads the Open rows at spec time exactly as before. No guard (§6), no question-legality rule (D6), no cap (D7), no depth-calibration text (§3.3), no signal class, no other brief section, and **no reader was changed by this document** — D-O58's line-4 reader is the orchestrator's, legislated there and referenced here. **No example was rewritten to fit the rule:** the worked example at §8 already read `OQ-1` and `OQ-2`, which is the evidence that the grammar was real before it was law.

| # | Decision | Ruling (adopted as stated) | Where |
|---|---|---|---|
| **D12** | Brief §6's Open-Question ID grammar, pinned as law | **`OQ-<n>`** — `OQ-1`, `OQ-2`, … — **numbered per brief**. The number is **that brief's own sequence**: it restarts in every brief and is **never globally unique**, because a brief is the unit a scoping call produces and a per-project sequence would make the ID a function of authoring order across epics. **Epic context rides BESIDE the ID in every render, never inside it** — the epic is the brief's own, every reader already carries it, and an epic-qualified ID (`E01-Q1`) would encode twice what is known once while breaking every matcher written against the grammar. **Stated once**, in §4's brief skeleton where the section's shape is pinned; the payload template and the Tier-1 skill carry that shape and cite it rather than restating it | §4 (brief skeleton, §6) |

**Conflict scan against standard v0.4 · contract v0.2 · gate v0.9 · orchestrator v0.23 · catalogues b1 v0.5, b2–b5 v0.2, b6 v0.4:** none found, and **nothing in any of them was changed**. The **gate's** CC-XA-06 reads brief §6's **Open rows** and their content, never their ID grammar — it asks whether an open question was resolved in the spec, not what the row was called. The **contract** legislates the spec-anchored estate and does not reach the brief. The **writing standard** owns the spec's headings and IDs, a different artifact with its own grammars. **Catalogue-b6's** Tier-1 sheet produces the brief and carries the skeleton by reference. The **orchestrator's** D-O58 is the one consumer of this grammar and cites it as a companion; it is stated here because **this document owns the brief's section shapes**. **No erratum issued.**




## 15. Review record (v0.7 → v0.8)

One sentence, ruled by the BA Lead 18 August 2026 — **principle 4 reads the negotiated statement as controlling.** Origin: orchestrator v0.27 (§30, EC-10). The frame now harvests negotiated engagement-scope decisions as `SD-<n>` entries with verbatim citations (D-O65), and the precedence principle rules they govern allocation, never discovery (D-O66). The two legitimacy tests — goal-blocking · hard-requested — live in this preamble, and catalogue-b6 cites them without re-legislating (its D-B6-8 discipline); the rule for *reading* them therefore lands here, once, and b6's D-B6-13 cites it. Ruled in the same sitting as the orchestrator edition by owner override of the one-session-one-document campaign rule.

| # | Decision | Ruling (adopted as recommended) | Where |
|---|---|---|---|
| **D13** | Principle 4's controlling-statement sentence | The two tests read a **negotiated engagement-scope decision** (orchestrator §8.1, D-O65–D-O66) as the **controlling client statement** where it conflicts with an earlier request in the material on hand — **composition-bounding only**: discovery is untouched, and a trimmed capability remains a cited row. One additive sentence at the principle's own line; no renumbering, no disturbed citation | Preamble principle 4 |

**Conflict scan against standard v0.4 · contract v0.2 · gate v0.10 · orchestrator v0.27 · catalogues b1 v0.6, b2–b5 v0.2, b6 v0.5:** none found, and **nothing else in any of them was changed by this edition**. The sentence bounds composition exactly where principle 4 already does; discovery and coverage-complete are untouched by its own words, and every guard, template row and skill brief stands byte-unchanged. **No erratum issued.**

---

## 16. Review record (v0.9 → v0.10)

One check, ruled by the BA Lead 25 August 2026 — **the pre-flight learns the marker grammar.** Origin: EC-21, the field design-defect report *the gate does not distinguish evidence from decision* (25 Aug 2026) and the triage at `docs/field-notes/2026-08-22-cc-fl-04-coverage-inversion.md`. A Tier-2 run in assumption posture minted `[ASSUMED: …]` into 45 error-table cells; the writing standard defined no such form, the pre-flight checked no marker shape, and the gate's stub test read the mint as content. The catching half is the contract's (CC-G-02's closed namespace) and the gate's (§5.2); the preventing half is this engine's, because the writer is the framework's own skill and D11 already draws the line: tolerance for specs the framework did not write, discipline for the ones it did.

| # | Decision | Ruling (adopted as recommended) | Where |
|---|---|---|---|
| **D14** | The shape pre-flight's fifth check — the marker grammar | Before the draft is written out, every bracketed token of marker shape — an upper-case label, with or without a colon — must be the spec's one marker, `[NEEDS CLARIFICATION: …]` (standard rule 7). A mint **stops the write** with found vs expected, at authoring, where it is one edit; at the gate the same mint is a **non-waivable** CC-G-02 line (contract v0.4), so the pre-flight asserts nothing the gate does not and merely moves the catch to the one moment the author is present. The canvas's `[CONFLICT: …]` is not a spec form and is not admitted. D11's *four checks* is amended to five **on the record here**, never by rewriting its row; §7.2's build requirement names the check. Alternative — leave prevention to rule 7's prose — **rejected**: the mint was made by a skill that had read the prose | §5.3 step 4 · §7.2 |

**Conflict scan against standard v0.6 · contract v0.4 · gate v0.12 · orchestrator v0.38 · catalogues b1 v0.7, b2–b5 v0.2, b6 v0.7:** none found, and **nothing else in any of them was changed by this edition**. The check reads rule 7's grammar exactly as the standard states it and admits exactly the namespace CC-G-02 admits for a spec; the confidence rule and cite-or-mark (§5.3) are untouched — the check enforces their form and restates nothing; no GQ packet, no cap, no guard and no BA step moves. **No erratum issued.**

---

## 17. Review record (v0.10 → v0.11)

One sentence, ruled by the BA Lead 4 September 2026 — **principle 4 carries one test.** Origin: **EC-24** — the field feedback of the AI-SDD team sync of early September 2026 (the Nutrivity run's first phase wider than the BA's four-screen cut; `presale-qr-code`'s allocation moved on money), triaged at `docs/field-notes/2026-09-04-first-phase-composed-wide.md`; the allocation-grain ruling is orchestrator v0.45's (D-O104–D-O106, §48) and the consumer catalogue-b6 v0.9's (D-B6-18–D-B6-20). The principle composed lean by its first clause and wide by its second: *hard-requested* admitted an epic on a citation, and a client brief hard-requests in breadth, so the second test seated most of the material — and the one counterweight, the scope frame's capacity advisory, moved nothing at `apply all` by ruling. The rule for what composition admits lives here, once (D9's register); the change therefore lands here, and b6 cites it.

| # | Decision | Ruling (adopted as recommended) | Where |
|---|---|---|---|
| **D15** | Principle 4's admission sentence — one test, and the request as a flag | What enters the first phase, an essential-scope set, or a story set passes **the necessity test** — the stated business goal cannot be reached without it (goal-blocking). The former second test, *hard-requested*, is **retired as an admission ground and survives as a flag**: a hard request the goal does not need is recorded ground and a named candidate, never a seat — a cited row on a later phase or a deferred line in its brief, the request on the record, named by the composing run (T-18's advisory at phase grain; the brief's Deferred section and the story set's deferred lines inside an epic) so the BA may direct it in. **Uniform across the three grains on purpose:** a lean phase composed of fat epics is not lean. D13's controlling-statement sentence stands with one word moved (*the test reads*). Alternative — narrow the test at allocation grain only and leave essential scope and stories on two tests — **rejected**: the breadth the sync named (*most of it not needed for a first phase*) lives inside epics as much as between them, and a principle stated once cannot carry two admission rules | Preamble principle 4 |

**Conflict scan against standard v0.6 · contract v0.5 · gate v0.15 · orchestrator v0.45 · catalogues b1 v0.8, b2–b5 v0.3, b6 v0.9 · the source-audit definition v0.6 · the map definition v0.1:** none found, and **nothing else in any of them was changed by this edition**. §5.3's drafting posture (D10) reads the principle by citation and moves with it; §3.2.D's sibling-boundary checks and kit Q3's lean are consumers, not restatements; §8.2's reschedule-in-place is exactly the deferred line the flag lands in; D9's row in §12 quotes the principle as ruled on 13 Aug 2026 and stands as history. The sentence bounds composition exactly where principle 4 already does; discovery and coverage-complete are untouched by its own words, and every guard, template row and skill brief stands byte-unchanged. **No erratum issued.**

---

*v0.11 · principle 4 carries one test — the necessity test seats, the former *hard-requested* test retired as an admission ground and surviving as a flag: a hard request the goal does not need recorded ground and a named candidate, never a seat, uniform across epics, essential scope and stories — applied 4 Sep 2026 (D15, §17; companions: orchestrator v0.45 · catalogue-b6 v0.9) · v0.10 · the pre-flight learns the marker grammar — the fifth check at §5.3 step 4: every bracketed token of marker shape the spec's one marker, a mint stopping the write at authoring with found vs expected, the same mint a non-waivable CC-G-02 line at the gate; §7.2's build requirement naming it; D11's four checks amended to five on the record — applied 25 Aug 2026 (D14, §16; companions: standard v0.6 · contract v0.4 · gate v0.12) · v0.8 · principle 4's controlling-statement sentence — the two tests reading a negotiated engagement-scope decision as the controlling client statement where it conflicts with an earlier request, composition-bounding only, discovery untouched — applied 18 August 2026 (D13, §15) · v0.7 · brief §6's ID grammar pinned as law — `OQ-<n>`, numbered per brief, the number the brief's own sequence and never globally unique · epic context beside the ID in a render, never inside it · stated once in §4's skeleton, cited by the template and the Tier-1 skill · no reader change, no status-vocabulary change (D4), CC-XA-06 untouched — applied 17 Aug 2026 (D12, §14) · v0.6 · the writer/reader coupling — the skeleton emitted from the template **file** (§5.3 step 2) · the **shape pre-flight** at authoring time, found vs expected (§5.3 step 4) · both as §7.2 build requirements — applied 14 Aug 2026 (D11, §13; companions: writing standard v0.4 · orchestrator v0.18 — D-O50) · v0.5 · lean scope posture — the lean-composition anchor as **principle 4** (preamble; stated once, cited corpus-wide) · the Tier-2 drafting posture, with the Presale anti-completion guard (§5.3 step 1) — applied 13 Aug 2026 (D9–D10, §12; companion: catalogue-b6 v0.4 — D-B6-10–D-B6-11) · v0.4 · defer disposition added 9 Aug 2026 (§5.4; orchestrator D-O18–D-O19 companion) · v0.3 20 July 2026 · runs on writing standard v0.6 + completeness contract v0.4 · emits signals to orchestrator rules (document 5) · delivers to gate definition (document 4) · decisions D1–D15 locked (§11 · §12 · §13 · §14 · §15 · §16 · §17) · all framework skills authored from scratch — build briefs in §7*
