...
### BA-Native Spec · gate contract · v0.5 — boundary coverage joins Scope H: every billable epic has a brief, and an uncovered one is a named gap (31 Aug 2026; review incorporated 19 July 2026)

**v0.5 change record:** one assertion row, ruled 31 Aug 2026 (origin: **EC-22** — the field defect report *scope escape — silent under-coverage* (31 Aug 2026, project `presale-qr-code`, Presale under a standing AG, `Boundary: MVP + Phase 2`) and its verified triage `docs/field-notes/2026-08-31-scope-escape-boundary-coverage.md`: 2 of 14 roadmap epics — both Phase 2, both inside the delivery boundary, both RFQ cost-breakdown line items — were never briefed or specced, ~160 eng-h ≈ 20% of the engagement absent from the client-facing WBS for three days, and no surface said so: **CC-H-03 is conditioned on *entering Band 3***, so an epic that never enters is invisible to it, and the batch driver, the health run and the export each operated on a subset with no way to know it was a subset; BA Lead ruling **`apply all`** over the sitting's D0–D7 decision list, 31 Aug 2026; base commit `8867df7`, package 0.1.46): **CC-H-08 — boundary coverage** joins §6 — every roadmap epic allocated to a phase inside the ledger head's `Boundary:` set (orchestrator §2.4; §10.5's own Billable test, D-O67) has a scope brief in `.specify/memory/scope/`; an epic without one is a **named gap carrying its phase and its Billable value**; **vacuous where no roadmap or no boundary stands** — an absent source renders `—`, never a gap (D-O67's law), so the pre-decomposition and boundary-less states pass by construction. Chk **M** — a file-existence join over roadmap rows × the brief folder × the head's `Boundary:` line, CC-H-03's own class. **CC-H-03 is untouched** — it owns a different question (the entered epic's brief carries a slicing) at a different moment (P-O8's precondition, orchestrator §8.4 leans on it by name); two questions, two rows. The runtime — grain, counting, effect, run points — is the gate's (v0.14 §10.2 · §10.4, the CC-H-02/CC-H-07 division of labor verbatim): a live gap counts in `n gaps` and **blocks nothing** — the roadmap ⇄ brief-set join at boundary grain sits in no feature's `deps(F)` — and `HA-<nn>` applies exactly as to any H gap. **Assertion count 62 → 63: 25 M · 38 A (2 ⚑).** Companions: orchestrator v0.42, D-O99–D-O101 — the Tier-1 election takes the same set, and the set displays at four sites, one computation · quickstart · the EC-22 field note.
...
| CC-H-08 | Boundary coverage: every roadmap epic allocated to a phase inside the ledger head's `Boundary:` set (orchestrator §2.4) has a scope brief in `.specify/memory/scope/`; an epic without one is named with its phase and its Billable value. Vacuous where no roadmap or no boundary stands — an absent source renders `—`, never a gap. | brief+roadmap+ledger head | 10.41 · 7.2 complete | M |
