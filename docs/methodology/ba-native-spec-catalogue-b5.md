...
| Preconditions | **—** — canvas §6/§9/§12, `constraints.md`, `context.md` and presale material ground every budget and convention row; the conditionality check (D-B5-3) runs on evidence, not on another technique's file, and **T-14 precedes T-15 by construction** so the constitution is a successor, never an input (§4 step 1). |
...
| Preconditions | `.specify/memory/roles-permissions.md` (**T-12**) — **hard**: the Authorization principle is seeded unconditionally (D-B5-4) and its **enforcement surface is that file**, which the Governance references table must resolve to, existing and stub-free at write time (§3, §4 steps 1 and 4) · `.specify/memory/design-standards.md` (**T-14**) — **soft**, and already conditional by **D-B5-3**: referenced where T-14 landed, omitted on the aspect record where no design/UX ground exists. Absent, the principle stands (D-B5-4 is unconditional) and its reference row is marked rather than resolved — a dangling reference is a planning defect surfaced now, never a silent write (orchestrator §6.1, D-O84). |
...
| Preconditions | `.specify/memory/competitive-analysis.md` (**T-07**) — **soft**: its Covers entries are **one of the four sweep patterns** (§2 Evidence triggers), beside canvas §7 function neighbors, canvas §8 connection sides and constraint-driven exclusions, each independently sufficient for the D-B5-5 seed minimum. Absent, the sweep runs on the remaining three and **says so** — fewer candidates, no unwritable cell, and the ≥ 1 exclusion still stands or the genuinely-empty boundary takes its AW (orchestrator §6.1, D-O84). |
