...
### BA-Native Spec · build blueprint · v0.3 (EC-16 installer integrity, 20 August 2026)
**v0.3 change record:** installer integrity — the supersession notice and the fenced-block hash, ruled 20 Aug 2026 (**D-P2-13…D-P2-15**, amendment record §8; base commit `3d378ee`, package 0.1.30; origin: **EC-16** — a field bootstrap install of 0.1.30 found two holes on the ⬒ side of the install: the pinned Spec Kit ships its own `/speckit-constitution` and advertises it at step 3.1 of the Next Steps panel printed inside our installer's own output, so the file D-P2-6 sets aside can be recreated from template outside the evidence chain; and the manifest whole-file-hashes the two fenced-block merge targets it does not own outright): **the supersession notice** — the mirror blocks name `/speckit-constitution` **superseded by `/ba-run t15`**, the guard living entirely on our side of the fence because Spec Kit's `SKILL.md` is pinned upstream content the framework never edits (**D-P2-13**; D-P2-6 extended on the record, never rewritten) · **the fenced-block hash rule** — for `AGENTS.md` and `CLAUDE.md` the ⬒ set hashes **the installer-owned fenced block, never the whole file**, closing the false ⬒-drift any pre-existing content or later user edit outside the fence reports today (**D-P2-14**) · **the empty-⬒-directory trace** — the install-time content policy states that `.gitkeep` is stripped at install and empty ⬒ directories do not survive a clone of the installed project; readers guard, writers `mkdir -p` (**D-P2-15**, ruled **(a)** by letter in the same sitting). §1.1's manifest row and layout notes, §2.6's two mirror rows and the D-P2-6 row carry the law; **no build unit, no count, no session boundary and no exit-test step moves** — §2.9's pinned 67 and §4's nine sessions stand as written. Methodology only: `install.sh` and `payload/mirror/` are the follow-up build pass, and §8 is what that pass builds to.
...
│  │                                     installed-file hash list — whole file for
│  │                                     the ⬒ files we own outright, fenced-block
│  │                                     content only for the two merge targets
│  │                                     (D-P2-14)
...
- The `CLAUDE.md` mirror is an **append-only fenced block** (`<!-- ba-native-spec:begin/end -->`): existing project content is never overwritten; re-install replaces only the block. `AGENTS.md` is created if absent, block-appended if present. Because the fence is the only part of either file the installer owns, the manifest hashes **the block's content, not the file** (D-P2-14): whole-file hashing of a merge target reports ⬒-drift for any pre-existing content or later user edit outside the fence, which is the project's own material and never the installer's to certify. On first contact the block *is* the file and the two readings coincide; a fence that has gone missing is real drift and reports as such.
...
| 12 | `AGENTS.md` mirror | Standard essentials: golden rules · §2 skeleton · EARS + tiered acceptance · banned words · reference-never-restate · **the superseded-command line** (`/speckit-constitution` → `/ba-run t15`, D-P2-13) |
| 13 | `claude-block.md` (CLAUDE.md fenced block) | Same essentials + the `/ba-*` command index + **the superseded-command line** (D-P2-13) |
...
| D-P2-6 | Install-time content policy | **Directories + templates only, zero content stubs** (absent ≡ stubbed for every AT/CC-H trigger; installer-made stubs would pollute the evidence); Spec Kit's default `constitution.md` set aside to `ba/speckit-defaults/` so T-15 births the real file. **Amended 20 Aug 2026 (D-P2-13, §8):** the set-aside is paired with a **supersession notice in the mirror blocks** — `/speckit-constitution` named superseded by `/ba-run t15` — because the pinned kit's own skill recreates the file from template when it is missing, which is precisely the state this policy creates. **Amended 20 Aug 2026 (D-P2-15, §8):** `.gitkeep` is stripped at install and empty ⬒ directories do not survive a clone of the installed project — readers guard, writers `mkdir -p` |
...
## 8. Amendment record — EC-16 (v0.2 → v0.3)

Origin: **EC-16 · installer integrity**, registered 20 August 2026 from a field bootstrap install of package 0.1.30 into a live presale project, verified against repo HEAD `3d378ee`. Two defects on the ⬒ side of the install, plus one footnote. Ruled by the BA Lead at registration, 20 August 2026 (ruling **(a)** on the candidate record) — **D-P2-13 and D-P2-14 carry the registered fix direction; D-P2-15 was ruled (a) by letter** in the same sitting. The candidate record is the ground and is cited here, never restated. **Methodology only:** `install.sh` and `payload/mirror/` are the follow-up build pass; the table below is what that pass builds to. Both decisions extend D-P2-6 and the manifest's own ⬒-set rule **on the record** — neither is rewritten.

| ID | Decision | Ruling |
|---|---|---|
| **D-P2-13** | `/speckit-constitution` bypasses D-P2-6 — the pinned kit ships its own path back to a template-born constitution | **The mirror blocks name the supersession.** `payload/mirror/AGENTS.md` and `payload/mirror/claude-block.md` — hence the fenced `<!-- ba-native-spec:begin/end -->` block in both `AGENTS.md` and `CLAUDE.md` — carry a **superseded-command line**: **`/speckit-constitution` is superseded by `/ba-run t15`**; the constitution is born from Band-1 evidence, never from a template. **The guard lives entirely on our side of the fence** — Spec Kit's `SKILL.md` is pinned upstream content (v0.12.5, D-P2-8) the framework never edits, and the kit's Next Steps panel, printed inside our own installer's output, advertises the skill at step 3.1. **Why a notice and not a check:** the escape has no post-hoc test. A raw stub is caught — `constitution.md` enters scope through CC-H-01's static core (gate §3's `deps(F)`; the scoped-run map pairs it with CC-H-06 at gate §10.2) and a stub fails **CC-G-02**'s stub test, whose two-step refuses a stub waiver outright (gate §7.1). A **filled** constitution passes CC-G-02, and `is_speckit_default()`'s byte-compare against `templates/constitution-template.md` then classes it as authored content — left untouched, and preserved across every re-install. Gate-satisfying governance ground, manufactured outside the evidence chain and indistinguishable afterwards; the only surface that closes **both** outcomes is the agent's instruction, before the act. Today the framework mentions the skill exactly once — the expected-layout row at `tests/layout.expected:34` — and no mirror block, `ba-*` skill or `.specify/ba/` file guards it |
| **D-P2-14** | The manifest hashes its own merge targets | **Fenced-block content, never the whole file.** In the manifest's ⬒ hash list, `AGENTS.md` and `CLAUDE.md` are hashed over **the content between `<!-- ba-native-spec:begin -->` and `<!-- ba-native-spec:end -->`** — the only part of either file the installer owns. **The defect this closes:** whole-file hashing of an append-merge target. The manifest already excludes mutable content by design — runtime-born ◇ content, the five ledgers and `specs/` are deliberately absent so the manifest does not rot on the first real act — and then hashes two files whose material outside the fence is the project's own: a pre-existing `CLAUDE.md`, an `AGENTS.md` the team already keeps, any later edit to either, each reporting **false ⬒-drift** against a manifest that never certified that material. The rule follows the ownership, not the path. Degenerate cases need no exception: on first contact the block is the whole file and the two readings coincide, and a fence that has gone missing is real drift |
| **D-P2-15** | Trace level for the empty-⬒-directory footnote riding EC-16 | **Ruled (a) — one line in the install-time content policy.** The D-P2-6 row carries it: `.gitkeep` is stripped at install and empty ⬒ directories do not survive a clone of the installed project — **readers guard, writers `mkdir -p`**. The ground: `.gitkeep` is stripped from both payload copies (`install.sh:239` · `:260`) and from the ⬒ hash list (`:380`), and `specs/` + `.specify/memory/scope/` are created empty (`:273`), so §1.1's `scope/` **⬒ dir** marking is true of the install and false of any later clone: git carries no empty directory. Cosmetic in effect, doc-level in kind — which is why the rejected alternative (**(b)**, a build-pass comment fix only) does not reach it. `install.sh:245`'s *"directories carry over even when empty"* is **accurate about the install act**: `mkdir -p` does lay the empty directory down in the target. What goes unstated is what becomes of the project afterwards, and that is a property of the ⬒ contract §1.1 asserts, not a comment-level fact — a ⬒ marking a clone silently voids is a doc-level claim, and the policy row is where that claim is made |

**Citations corrected on the record.** The field report attributed the stub test to CC-H-01 (`assertions-f.md:32`). The stub test is **CC-G-02**; **CC-H-01** is the cross-check that pulls `constitution.md` into `deps(F)`. Both citations stand as corrected above, and the correction is the reason D-P2-13's rationale reads as two separate mechanisms rather than one.

**Deliberately not legislated here.** The Next Steps panel is Spec Kit's own output, reprinted by our installer; whether `install.sh`'s closing print should carry a counter-line beside it is an installer-output question, adjacent to D-P2-13 and outside the fix direction this amendment was authored to. Named, not ruled.

---

*v0.3 · EC-16 installer integrity applied 20 August 2026 (D-P2-13 · D-P2-14 · D-P2-15 ruled — §8) · v0.2 · review incorporated 30 July 2026 · Phase-2 opening deliverable per plan §0 next-action · compiles from: plan v2.13 · standard v0.3 · contract v0.2 · elicitation v0.3 · gate v0.3 · orchestrator v0.3 · seq-plan v0.4 · catalogue b1 v0.3, b2–b6 v0.2 · index v0.2 · environment pins verified 30 Jul 2026: Claude Code merged-skills model · Spec Kit v0.12.5 · consumed by: the S1–S9 build sessions (§4) and the plan §0 tracker · replace-on-update, stable file name*
