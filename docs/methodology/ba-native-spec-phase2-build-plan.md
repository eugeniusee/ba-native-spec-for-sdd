...
| Skill/command names allow **lowercase letters, digits, hyphens only** — the command comes from the directory name | The corpus's indicative dotted `/ba.*` names cannot be skill names; the hyphenated `ba-*` namespace replaces them (D-P2-1). Both binding tables said "names indicative — Phase 2 fixes them"; this plan fixes them |
