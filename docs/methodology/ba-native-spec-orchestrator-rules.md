...
### BA-Native Spec · aspect & band machinery · v0.16 — §11's floor row brought to four acts (the D113 carry item, closed) (13 Aug 2026)
...
The act (BA): initialize the ledgers — aspect-state head at six × `untouched`, `Band: 1 (open)`; empty plans file — **pick the flow profile (P-O0 — flow-profile selection; §6.5) and set the scope frame (P-O0b — scope-frame selection) against the two pinned blocks below** — and confirm `canvas.md` is present and carried into the repo: the presale canvas is Band 1's substrate (Q1a). **No canvas** (a non-presale entry): producing one is the first Frame act — a custom-contract technique run before any aspect opens, contract {presale canvas incl. Context/Constraints element · Context · `canvas.md`}; its sections then serve as the aspects' shared substrate (part of D-O7, locked). With the substrate, the profile, and the scope frame in place, Stakeholders — the root — is openable (T1).
...
**One render, one reply (D-O42).** The profile picker and the scope-frame block render **together, as the Frame act's single stop**: the BA answers both in one reply — the profile pick, then the frame confirmed or corrected. Frame therefore costs **one** BA interaction, not two, and §6.5's ≤ 8 Presale budget with its one interaction of slack stands unchanged (D-O33, untouched).

**Auto-pickup.** Before rendering the block the framework scans the sources on hand — client documents, a Slack extract, the canvas — for budget and scope constraints and **pre-fills the values with their citations**; the BA confirms or corrects. Cite-or-mark applies (doc 3's principle 3 — cited, marked, or asked, never guessed): every value carries a citation or an explicit `open — no source material`, T-01's own Frame-time token (catalogue-b1). **`none stated` is a legal, recorded answer** — it becomes a named client question, never silence.
...
| Frame + profile + scope frame | `/ba.frame` — ledger init → the §8.1 pinned profile picker (P-O0) → the §8.1 pinned scope-frame block (P-O0b), auto-pickup pre-filled from the sources on hand → canvas confirmation; the frame's fields and the derived `Capacity:` line land in the §2.4 head, switches append to Events |
