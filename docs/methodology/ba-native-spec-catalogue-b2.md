...
| Preconditions | `.specify/memory/stakeholders.md` (**T-03**) — **hard**: TC-1 pins each charter's Details cell to exactly one register population, resolving to a register entry (§5, D-B2-2), and the election-time population choice is made against that table. Absent, the charters are drafted against the canvas Customers populations with every Details cell marked, and TC-1's anchor stands open until the register lands (orchestrator §6.1, D-O84). |
...
| Preconditions | **—** — canvas §8 and the Context/Constraints element plus presale material ground the Existing-systems and landscape cells; the register is read for organizational colour, never for a contracted cell. |
...
| Preconditions | **—** — canvas §13's one-liners and presale material ground every class row; `context.md` is read for the landscape-vs-bind boundary, and its absence costs routing candidates, never a cell (a class with no candidate takes its `none identified — <basis>` line, as always). |
...
| Preconditions | **—** — canvas Unlike/Problems/Objectives, presale material and transcripts ground every entry; the set may legally be `N/A — <reason>` on the BA's ruling. |
