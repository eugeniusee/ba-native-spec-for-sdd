...
### BA-Native Spec · Wave 2 · Requirements core chain · v0.3 — the sheets declare what they take in (21 Aug 2026; review incorporated 29 July 2026)
**v0.3 change record:** one template amendment, ruled 21 Aug 2026 (**R3** of the EC-18 package; origin: the field defect report of 20 Aug 2026, **Part B item B7** — electing T-12 alone silently pulled in `domain-model.md` (T-11 output) and `constitution.md` (T-15 output), and the dependency was declared nowhere the election could see it): **every sheet in this batch gains the §2 metadata's fifth field, `Preconditions`** (sequencing plan v0.5 §2) — the other techniques' artifacts this sheet's own contract and template text demand, each with its producing technique and its grade, **hard** (a contracted cell cannot be authored without it) or **soft** (one input among several; absence costs candidates, not cells), and **`—` where the sheet takes none**, with the reason stated so a bare dash is never a silence. What the field does at an election is orchestrator law and is cited, never restated: **visibility, never a block** — the election sees what it is taking on, nothing is silently pulled in, and the run proceeds in assumption posture with the dependent cells marked (orchestrator v0.33 §6.1, **D-O84**). **T-12 is the reported case: `domain-model.md` (T-11) hard** — the Policy table's entity cells are verbatim from that file (§3, §4 step 4, §5 template; CC-H-05 at authoring time) — **and `constitution.md` (T-15) soft**, the authorization principle named here and authored there, the sheet already disclaiming the authorship (§1, §5 template). **T-13 declares both hard** — role cells verbatim from `roles-permissions.md` (T-12), entities by `domain-model.md` name (T-11), the two halves of its own §4 step-4 coherence pass. **T-11 declares `context.md` (T-05) soft** — the Boundary-references pointer, which canvas §8 alone satisfies. The chain's ordering statements in §4 step 1 stand exactly as written; the field puts the same fact where the **election** reads it, which §4 step 1 never was. **No purpose, trigger, skip-if, depth, output contract, procedure step, template, micro-example, gate hook, mining note or build-brief hook moved**, and no decision in this batch is re-ruled: the field **declares what the sheets already demanded** and adds no demand of its own.
...
| Preconditions | `.specify/memory/context.md` (**T-05**) — **soft**: §3's Boundary-references list disposes each EG-1 connection system as external **pointing at its `context.md` / canvas §8 home**, and canvas §8 alone satisfies the pointer — absent, the rows point at the canvas and say so. The entity and relation tables, which are the contract's substance, are grounded on canvas §§7–8 (EG-1, D-B3-5) and need nothing else (orchestrator §6.1, D-O84). |
...
| Preconditions | `.specify/memory/domain-model.md` (**T-11**) — **hard**: the Policy table's entity cells are **verbatim from that file** (§3, §4 step 4, §5 template; CC-H-05 applied at authoring time), so without it every entity cell is a mark rather than a citation and the tuple set cannot be screened against a reference surface · `.specify/memory/constitution.md` (**T-15**) — **soft**: the authorization *principle* is named here and authored there (§1, §5 template line; plan §4.14), and this file is its enforcement surface either way — absent, the template's principle line marks the reference and the policy rows stand unchanged. Both render at the election and neither blocks it (orchestrator §6.1, D-O84). |
...
| Preconditions | `.specify/memory/roles-permissions.md` (**T-12**) — **hard**: every journey's role cell is **verbatim from that file** (§2 Depth, §3, §5 template heading) and step 4's coherence pass resolves against it · `.specify/memory/domain-model.md` (**T-11**) — **hard**: the numbered steps touch **entities by domain-model name** (§3, §5 template), the second half of the same coherence pass. Absent, the journeys are drafted from the canvas Core Functions actors and function objects with the role and entity cells marked, and the coherence pass records what it could not resolve; neither absence blocks the run (orchestrator §6.1, D-O84). |
