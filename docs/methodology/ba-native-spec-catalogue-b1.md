...
### BA-Native Spec · Wave 2 · Frame & foundation · v0.8 — the sheets declare what they take in (21 Aug 2026; cross-cutting mirror 19 August 2026; B3 mirrors 30 July 2026; review incorporated 25 July 2026)
**v0.8 change record:** one template amendment, ruled 21 Aug 2026 (**R3** of the EC-18 package; origin: the field defect report of 20 Aug 2026, **Part B item B7** — electing T-12 alone silently pulled in `domain-model.md` (T-11 output) and `constitution.md` (T-15 output), and the dependency was declared nowhere the election could see it): **every sheet in this batch gains the §2 metadata's fifth field, `Preconditions`** (sequencing plan v0.5 §2) — the other techniques' artifacts this sheet's own contract and template text demand, each with its producing technique and its grade, **hard** (a contracted cell cannot be authored without it) or **soft** (one input among several; absence costs candidates, not cells), and **`—` where the sheet takes none**, with the reason stated so a bare dash is never a silence. What the field does at an election is orchestrator law and is cited, never restated: **visibility, never a block** — the election sees what it is taking on, nothing is silently pulled in, and the run proceeds in assumption posture with the dependent cells marked (orchestrator v0.33 §6.1, **D-O84**). All three sheets declare **`—`**: T-01 runs before any context stack exists, T-02 sweeps whatever estate stands, and T-03 grounds every cell on the canvas and presale material. **No purpose, trigger, skip-if, depth, output contract, procedure step, template, micro-example, gate hook, mining note or build-brief hook moved**, and no decision in this batch is re-ruled: the field **declares what the sheets already demanded** and adds no demand of its own.
**v0.7 change record:** one additive mirror line, ruled 19 Aug 2026 (**D-B1-8**, review record below; origin: orchestrator v0.29 §32 — the cross-cutting obligations register, EC-01 + owner ruling Р8; D-O72–D-O74): the D-B1-5 scope-frame mirror extends by the **cross-cutting line** — where the ledger head carries standing `XO-<n>` entries (orchestrator §2.4), they land in canvas §13 Context/Constraints as cited lines beside budget envelope · delivery boundary · client label · scope decisions — class and one-line value, each with its verbatim citation; the language line's engagement default carries as `language: English (engagement default)` — framework law recorded as its own ground, never a fabricated citation (orchestrator D-O74) — the head holding the machine-readable summary and the canvas the cited detail. **Mirror only** — T-01 neither sets nor edits the register, and **D-B1-6's fence stands**: the harvest is Frame-act ground, never this run's; T-01 asks nothing. §4 step 2, the §5 template's §13 row and the §8 build hook extend in step; no section, line-ID scheme, AT hook or micro-example moved. Companion: orchestrator v0.29 (D-O72–D-O77). T-02 and T-03 untouched.
**v0.6 change record:** one additive mirror line, ruled 18 Aug 2026 (**D-B1-7**, review record below; origin: orchestrator v0.27 §30 — the scope-decision harvest, D-O65): the D-B1-5 scope-frame mirror extends by the **scope-decisions line** — where the frame carries standing `SD-<n>` entries (orchestrator §2.4), they land in canvas §13 Context/Constraints as cited lines beside budget envelope · delivery boundary · client label, the head holding the machine-readable summary and the canvas the cited detail. **Mirror only** — T-01 neither sets nor edits the frame, and **D-B1-6's fence stands**: the harvest is Frame-act ground, never this run's; T-01 asks nothing. §4 step 2, the §5 template's §13 row and the §8 build hook extend in step; no section, line-ID scheme, AT hook or micro-example moved. Companions: orchestrator v0.27 (D-O65–D-O67) · catalogue-b6 v0.5 (D-B6-12–D-B6-13) · elicitation v0.8 (D13). T-02 and T-03 untouched.
**v0.5 change record:** one additive input clause, ruled 13 Aug 2026 (**D-B1-6**, review record below; the source inventory — origin in orchestrator §21): the source artifacts the Frame act's inventory captured (`sources/<origin>-<date>.md`; orchestrator §8.1, D-O46–D-O47) join T-01's **material on hand** and its §8 build-hook input list, read exactly like a supplied transcript and **cited by artifact, never by the live channel**. The boundary is stated once: the inventory is **Frame-act ground and never this run's** — T-01 neither names a source nor rules a disposition, and the no-question-loop rule of §2's Depth cell is untouched. **Inputs only** — the framing report's per-section sourcing already covers cite-or-mark over captures, so no procedure step, section, line-ID scheme, AT hook or micro-example moves. Companion: orchestrator v0.17 (D-O45–D-O49) · catalogue index v0.5, regenerated with zero cell changes. T-02 and T-03 untouched.
**v0.4 change record:** one additive mirror clause, ruled 13 Aug 2026 (**D-B1-5**, review record below; the scope frame — origin in orchestrator §20): T-01 carries a standing scope frame's detail into canvas §13 Context/Constraints as cited lines — budget envelope · delivery boundary · client label — the head holding the machine-readable summary and the canvas the cited detail; the §5 template's §13 row names the landing site and the §8 build hook names the source. Mirror only — T-01 neither sets nor edits the frame. Companions: orchestrator v0.15 (D-O42–D-O44) · catalogue-b6 v0.3 (D-B6-7…D-B6-9) · index v0.3. T-02 and T-03 untouched; no section, line-ID scheme, AT hook, or micro-example moved.
...
| Preconditions | **—** — Frame is first and no context stack exists yet: the inputs are presale material as supplied plus the captured `sources/` and the ledger head's scope-frame fields, none of them a technique output (§8; D-B1-6…D-B1-8). |
...
### 4. Procedure
1. **BA act:** at Band-1 entry (Frame, orchestrator §8.1), invoke T-01 (P-O3) with all presale material on hand — canvas export, briefs, decks, transcripts — or nothing (greenfield conversation). The run is recorded under the `Frame` section of `aspect-plans.md` (D-B1-4, locked). **Captured sources (D-B1-6, locked):** the material on hand includes the source artifacts the Frame act's source inventory captured — one file per capture under `sources/` at repo root (orchestrator §8.1, D-O46–D-O47) — read exactly like a supplied transcript and cited by artifact, never by the live channel. The inventory is **Frame-act ground and never this run's**: T-01 neither names a source nor rules a disposition, and asks nothing — the no-question-loop rule of §2's Depth cell is untouched by this input. Holes stay visibly open.
2. **Framework act:** if a presale canvas exists — parse it into the output template, content carried verbatim per section, each carried line tagged `[presale canvas]`; other supplied material is mined (10.18) with its own citations. If none exists — pre-draft all 13 sections from what the BA supplied, cite-or-mark per line (principle 3). **Scope-frame mirror (D-B1-5, locked):** where a scope frame stands (orchestrator §8.1 — P-O0b, set before this run), its detail carries into §13 Context/Constraints as cited lines — budget envelope · delivery boundary · client label · scope decisions (`SD-<n>`, each with its verbatim citation — D-B1-7) · cross-cutting obligations (`XO-<n>` — class and one-line value, each with its verbatim citation; the language default as `language: English (engagement default)`, framework law — D-B1-8) — each with its source or an explicit `open — no source material`. The ledger head holds the machine-readable summary (orchestrator §2.4); the canvas holds the cited detail. Mirror only: this run neither sets nor edits the frame.
...
| 13 | Context/Constraints | one-liners per class (technical · business · regulatory); detail owned by `constraints.md` (#5) from the Context aspect on; scope-frame lines land here where a frame stands — budget envelope · delivery boundary · client label · scope decisions (`SD-<n>`) · cross-cutting obligations (`XO-<n>`), each cited (D-B1-5 · D-B1-7 · D-B1-8) |
```
...
Invocation: `/ba-run T-01` at the Frame act — the one catalogue technique invoked outside an aspect plan (D-B1-4, locked: recorded under a `Frame` section of `aspect-plans.md`); contract check at P-O3 (orchestrator §11). Inputs loaded: presale material as supplied, files of any format — no context stack exists yet; Frame is first; plus the captured source artifacts under `sources/` where the inventory captured any (orchestrator §8.1 — D-B1-6); plus the ledger head's scope-frame fields where a frame stands (orchestrator §2.4 — the D-B1-5 mirror's source, the `Scope decisions:` and `Cross-cutting:` lines included — D-B1-7 · D-B1-8). Interaction: parse-or-draft → framing report → BA review + N/A rulings → write + §8.1 confirmation. Outputs written: `canvas.md` only; the framing report is session output. §§2–6 above fix structure, contract, procedure, and depth; Phase 2 adds multi-format file ingestion, parse-vs-draft branch detection, P-O3 wiring, and the framing-report rendering.
...
| Preconditions | **—** — the sweep surface is *whatever `.specify/memory/` artifacts exist* plus the canvas; no contracted cell is bound to a named technique's output, and a thin estate costs terms to sweep, never a cell to write. |
...
| Preconditions | **—** — canvas Customers, presale material and transcripts ground every contracted cell; no other technique's artifact is demanded. |
...
## Review record (v0.4 → v0.5)

One decision ruled by the BA Lead, 13 August 2026 — the source inventory's T-01 half. Origin and the orchestrator half: orchestrator §21 (D-O45–D-O49).

| # | Decision | Ruling (adopted as stated) | Basis |
|---|---|---|---|
| **D-B1-6** | Captured sources join T-01's material on hand — inputs only | **One additive input clause, no restructuring.** The source artifacts the Frame act's inventory captured — one file per capture under **`sources/` at repo root** (orchestrator §8.1, D-O46–D-O47) — join the material on hand at §4 step 1 and the §8 build hook's input list, **read exactly like a supplied transcript and cited by artifact, never by the live channel**. The boundary, stated once and only once: the inventory is **Frame-act ground and never this run's** — T-01 neither names a source nor rules a disposition, and **asks nothing**; §2's Depth cell (*T-01 runs no question loop at all*) is untouched by this input. Holes stay visibly open and remain the suggestion engine's material. **No procedure change beyond the input clause** — the framing report's per-section sourcing already covers cite-or-mark over captures | Orchestrator §8.1 (D-O45–D-O47), §2.4 (D-O48); D-B1-2 (the two Frame-time states); principle 3. Alternative — T-01 taking the inventory as a question step — rejected by the ruling itself: a question loop at Frame is the depth rule this sheet has fenced since v0.1, and the inventory's dispositions are BA rulings, not technique output |

**Conflict scan — against the spine (standard v0.3 · contract v0.2 · elicitation v0.5 · gate v0.6 · orchestrator v0.17) and catalogue-b6 v0.4.** Checked: the clause is additive at §4 step 1 and in the §8 hook's input list, and steps 2–6 are byte-unchanged — including D-B1-5's scope-frame mirror at step 2 · the §5 output template, the 13 sections and D-B1-1's line-ID scheme are untouched, because a capture is an **input** and never a canvas section · §2's Depth cell and its no-question-loop sentence are unchanged, and the new clause cites them rather than qualifying them · §3's Destination file cell still reads `canvas.md` — `sources/` is an input home, never this sheet's output · `sources/` joins no estate glob, so §6's Health hook (CC-H-01 via `canvas.md`) and every AT hook stand · **the micro-example is deliberately not extended** — the pre-RO-1 appointment-booking world is pinned by the 25 July conflict scan, and the engagement had no inventoried source. **No spine contradiction found; no erratum issued.**

## Review record (v0.3 → v0.4)

One decision ruled by the BA Lead, 13 August 2026 — the scope frame's T-01 half. Origin and the orchestrator half: orchestrator §20 (D-O42–D-O44); the T-18 half is catalogue-b6's D-B6-7…D-B6-9.

| # | Decision | Ruling (adopted as stated) | Basis |
|---|---|---|---|
| **D-B1-5** | The scope-frame mirror — the frame is set at P-O0b before T-01 runs, and its detail needs a cited home | **One additive mirror clause, no restructuring.** Where a scope frame stands (orchestrator §8.1 — P-O0b), the framing run carries its detail into **canvas §13 Context/Constraints** as cited lines — budget envelope · delivery boundary · client label — each with its source or an explicit `open — no source material`, T-01's own Frame-time token. **The ledger head holds the machine-readable summary** (orchestrator §2.4, D-O43); **the canvas holds the cited detail.** Mirror only: this run neither sets nor edits the frame — P-O0b is a safety-floor act and the frame is the BA's (orchestrator §10.7). The §5 template's §13 row names the landing site and the §8 build hook names the source | Orchestrator §8.1, §2.4, §10.7; D-B1-2 (the two Frame-time states, `open — no source material` vs. `N/A — <reason>`); principle 3. Alternative — the canvas as the frame's home of record — rejected: the machinery reads the head, and a second authoritative copy is the tracking-split failure the corpus fences everywhere else |

**Conflict scan — against the spine (standard v0.3 · contract v0.2 · elicitation v0.4 · gate v0.6 · orchestrator v0.15) and catalogue-b6 v0.3.** Checked: the clause is additive at §4 step 2, and steps 1 and 3–6 are byte-unchanged · the §5 template's §13 row gains a pointer and no section is added, renamed, or reordered — the 13 sections and D-B1-1's line-ID scheme stand · §13's existing owner sentence is intact: detail is `constraints.md`'s from the Context aspect on, and the frame's lines are cited Frame-time content, not constraint rows (T-06's ground is untouched) · **the micro-example is deliberately not extended** — the pre-RO-1 appointment-booking world is pinned by the 25 July conflict scan, and the engagement had no recorded frame · no AT criterion, no gate hook, and no contract cell moves. **No spine contradiction found; no erratum issued.**



## Review record (v0.5 → v0.6)

One additive mirror line, ruled by the BA Lead 18 August 2026. Origin: orchestrator v0.27 (§30 — EC-10; D-O65). The frame now carries negotiated engagement-scope decisions as `SD-<n>` entries with verbatim citations; a frame line without its canvas mirror would break the head-summary / canvas-detail split D-O43 set and D-B1-5 carries. Ruled in the same sitting as the orchestrator edition by owner override of the one-session-one-document campaign rule.

| # | Decision | Ruling (adopted as recommended) | Where |
|---|---|---|---|
| **D-B1-7** | The canvas mirror carries scope decisions | The D-B1-5 mirror extends by one line: standing `SD-<n>` entries land in canvas §13 Context/Constraints as cited lines beside budget envelope · delivery boundary · client label — the ledger head holding the machine-readable summary, the canvas the cited detail. **Mirror only:** T-01 neither sets nor edits the frame, and **D-B1-6's fence stands untouched** — the harvest is Frame-act ground, never this run's; T-01 asks nothing | T-01 §4 step 2 · §5 template §13 row · §8 build hook |

**Conflict scan — against the spine (standard v0.4 · contract v0.2 · elicitation v0.8 · gate v0.10 · orchestrator v0.27) and catalogue-b6 v0.5.** Checked: the line is additive at the three D-B1-5 sites and nothing else moves — every other step and section byte-unchanged, no line-ID scheme, AT hook or micro-example touched · **D-B1-5 and D-B1-6 are extended and consumed on the record, never rewritten** · the contract's checks read the canvas as a whole and pin no §13 line set, so no assertion moves. **No contradiction found; no erratum issued.**

## Review record (v0.6 → v0.7)

One additive mirror line, ruled by the BA Lead 19 August 2026. Origin: orchestrator v0.29 (§32 — EC-01 + owner ruling Р8; D-O72–D-O74). The ledger head now carries cross-cutting obligations as `XO-<n>` entries with verbatim citations; a head register without its canvas mirror would break the head-summary / canvas-detail split D-O43 set and D-B1-5 carries — the D-B1-7 reasoning, applied to the new line. Ruled in the same sitting as the orchestrator edition by owner override of the one-session-one-document campaign rule.

| # | Decision | Ruling (adopted as recommended) | Where |
|---|---|---|---|
| **D-B1-8** | The canvas mirror carries cross-cutting obligations | The D-B1-5 mirror extends by one line: standing `XO-<n>` entries land in canvas §13 Context/Constraints as cited lines beside budget envelope · delivery boundary · client label · scope decisions — class and one-line value with the verbatim citation; the language line's engagement default carries as `language: English (engagement default)` — framework law recorded as its own ground, never a fabricated citation (orchestrator D-O74) — the ledger head holding the machine-readable summary, the canvas the cited detail. **Mirror only:** T-01 neither sets nor edits the register, and **D-B1-6's fence stands untouched** — the harvest is Frame-act ground, never this run's; T-01 asks nothing | T-01 §4 step 2 · §5 template §13 row · §8 build hook |

**Conflict scan — against the spine (standard v0.4 · contract v0.2 · elicitation v0.8 · gate v0.10 · orchestrator v0.29) and catalogue-b6 v0.6.** Checked: the line is additive at the three D-B1-5 sites and nothing else moves — every other step and section byte-unchanged, no line-ID scheme, AT hook or micro-example touched · **D-B1-5, D-B1-6 and D-B1-7 are extended and consumed on the record, never rewritten** · the contract's checks read the canvas as a whole and pin no §13 line set, so no assertion moves. **No contradiction found; no erratum issued.**

---

*v0.7 · the canvas mirror carries cross-cutting obligations, applied 19 August 2026 (D-B1-8 — T-01 §4 step 2, the §5 §13 template row, the §8 build hook; D-B1-6's fence untouched; review record above) · v0.6 · the canvas mirror carries scope decisions, applied 18 August 2026 (D-B1-7 — T-01 §4 step 2, the §5 §13 template row, the §8 build hook; D-B1-6's fence untouched; review record above) · v0.5 · captured sources join T-01's inputs, applied 13 August 2026 (D-B1-6 — T-01 §4 step 1, the §8 build hook; the question-free boundary stated once; review record above) · v0.4 · the scope-frame mirror applied 13 August 2026 (D-B1-5 — T-01 §4 step 2, the §5 §13 template row, the §8 build hook; review record above) · v0.3 · B3 mirrors applied 30 July 2026 (D-B3-1 · D-B3-3) · review incorporated 25 July 2026 · sheets T-01–T-03 of 18 · authored to sequencing-plan §2 template (D-W9 order: contract before procedure) · ★ mining pattern established at T-01 · D-W8 executed — T-02 to the no-reference pattern; plan erratum ready to apply · decisions D-B1-1…D-B1-8 locked (review records above) · consumed by: batches B2–B6 (cite, never restate) · next: B2 — Stakeholders & Context breadth (T-04…T-07), standing batch prompt §4*
