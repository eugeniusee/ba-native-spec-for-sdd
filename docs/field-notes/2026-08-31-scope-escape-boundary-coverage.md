...
**Status:** **fixed at the methodology layer, 31 Aug 2026** — orchestrator **v0.42** (D-O99–D-O101, §45) · completeness contract **v0.5** (CC-H-08) · gate **v0.14** (§10.1 · §10.2 · §10.4), then **v0.15** (the CC-H-08 line's §7 form) · quickstart. **Compiled at package 0.1.49, 1 Sep 2026** — 20/20 green; four divergences registered by the compile pass and ruled the same day (§6). The fix reaches project estates **by release only**.
...
- Killed string *"every epic allocated to the first phase"*: **exactly 2** occurrences in the orchestrator document, both historical and byte-deliberate (D-O61's register row, untouched, and the v0.42 change record's origin quotation) · **0** in all six carriers · **0** in the quickstart. The `ba-orchestrator` drift variant (*"every epic in the first phase"*): **0** in every carrier and the quickstart; the orchestrator document quotes it twice, both in the v0.42 records (change record and §45), byte-deliberate.
...
## 6. Compile pass — divergences registered and ruled (package 0.1.49, 1 Sep 2026)

The pass compiled items 1, 3–11 of the Lane A prompt as ruled — five carriers, `sk_health.py`'s fourth M row (`boundary_coverage()` in the gate, imported by `sk_status` and `sk_wbs`, its grounds imported from `sk_wbs` so the set is the rows the export bills), the four display sites, D-O101's join, the humanizer guard's two pins, the gate surfaces count-free, the edition pins, and two fixtures (`tests/fixtures/qr-boundary/` — the field case's shape; Fixture B the same estate under `Boundary: MVP`, the set asserted identical to the first-phase list, byte-identity by construction). Four divergences were registered, never resolved silently; all four ruled `apply all`, 1 Sep 2026:

1. **Drift-variant census** — this note's §5 said *0 everywhere*; the orchestrator document quotes the variant twice, both historical. Doc wins; §5 corrected above.
2. **The gap line** — gate §10.4 illustrated the line without contract §7's `: <what is wrong> → <fix action>`; §7 rules a fix-less line invalid gate output. The pass took §7's form with every illustrated token present and in order; **gate v0.15** documents it, the illustrated prefix and its pins unchanged.
3. **The card** — `assertions-h.md` is compiled and carries the analyst's A rows only; CC-H-08 is M. Its runtime lands in `sk_health`'s M list and `SCOPE_H`; `check-gate` asserts the card does not carry it. Contract §6's row is the law; no document changes.
4. **The map's 24 stays** — `/ba-map`'s Spec-Kit compliance denominator is the per-feature-evaluable M set; a Scope-H assertion no per-feature gate report evaluates must not drop every certified feature to 96%. Map definition D-M2's now-stale gloss is **routed to master**, amended at the next map sitting.

Six judgement calls stand in the BUILD-LOG entry for 0.1.49. Defaults taken: none.
